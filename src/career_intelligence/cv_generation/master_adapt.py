    ...
    *,
    ...
    for heading, body in sections["__order__"]:
        ...
        if key == _PROJECTS_HEADING:
            body_parts.append(_render_projects_section(body, ordered_names))
            continue
    ...
    text = re.sub(r"\n{3,}", "\n\n", text).rstrip() + "\n"
    # Drop Master-only canonical footer.
    text = re.sub(
...
def _render_projects_section(master_projects_body: str, ordered_names: list[str]) -> str:
